...
if (
    isset($field["value"]) &&
    $field["value"] instanceof \Carbon\CarbonInterface
) {
    $field["value"] = $field["value"]->format("Y-m-d H:i:s");
}

$field_language = isset($field["datetime_picker_options"]["language"])
    ? $field["datetime_picker_options"]["language"]
    : \App::getLocale();
?>
    ...
    <script>
        ...
        function bpFieldInitDateTimePickerElement(element) {
                ...
                $picker.on('dp.change', function(e){
                    var sqlDate = e.date ? e.date.format('YYYY-MM-DD HH:mm:00') : null;
                    $field.val(sqlDate);
                });
        }
    </script>
