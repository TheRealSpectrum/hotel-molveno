@extends(backpack_view('layouts.plain'))

@section('content')
    <style>
        #pw-container {
           position: relative;
        }

        #pw-eye {
            position: absolute;
            cursor: pointer;
            right: 10px;
            top: 12px;
        }

        #pw-eye-slash {
            position: absolute;
            cursor: pointer;
            display: none;
            right: 10px;
            top: 12px;
        }

        .img-container {
            display: flex;
            justify-content: center
        }
    </style>
    <div class="row justify-content-center">
        <div class="col-12 col-md-8 col-lg-4">
            <div class="img-container">
                <a href="/">
                    <img src="{{ url("images/Logo Molveno Resort Black.svg") }}" alt="Molveno Resort Logo" height="150" width="150">
                </a>
            </div>
            <h3 class="text-center mb-4">Admin {{ trans('backpack::base.login') }}</h3>
            <div class="card">
                <div class="card-body">
                    <form class="col-md-12 p-t-10" role="form" method="POST" action="{{ route('backpack.auth.login') }}">
                        {!! csrf_field() !!}

                        <div class="form-group">
                            <label class="control-label" for="{{ $username }}">{{ config('backpack.base.authentication_column_name') }}</label>

                            <div>
                                <input type="text" id="uname-input" class="form-control{{ $errors->has($username) ? ' is-invalid' : '' }}" name="{{ $username }}" value="{{ old($username) }}" id="{{ $username }}">

                                @if ($errors->has($username))
                                    <span class="invalid-feedback">
                                        <strong>{{ $errors->first($username) }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>

                        <div class="form-group">
                            <label class="control-label" for="password">{{ trans('backpack::base.password') }}</label>

                            <div id="pw-container">
                                    <input type="password" id="pw-input" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" id="password">
                                    @if (!$errors->has("password"))
                                        <i id="pw-eye" onclick="passvisToggle()" class="las la-eye"></i>
                                        <i id="pw-eye-slash" onclick="passvisToggle()" class="las la-eye-slash"></i>
                                    @endif
                                @if ($errors->has('password'))
                                    <span class="invalid-feedback">
                                        <strong>{{ $errors->first('password') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>

                        <div class="form-group">
                            <div>
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" name="remember"> {{ trans('backpack::base.remember_me') }}
                                    </label>
                                </div>
                            </div>
                        </div>

                        <div class="form-group">
                            <div>
                                <button type="submit" class="btn btn-block btn-primary" onclick="removeEye()">
                                    {{ trans('backpack::base.login') }}
                                </button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
            @if (backpack_users_have_email() && config('backpack.base.setup_password_recovery_routes', true))
                <div class="text-center"><a href="{{ route('backpack.auth.password.reset') }}">{{ trans('backpack::base.forgot_your_password') }}</a></div>
            @endif
            @if (config('backpack.base.registration_open'))
                <div class="text-center"><a href="{{ route('backpack.auth.register') }}">{{ trans('backpack::base.register') }}</a></div>
            @endif
        </div>
    </div>
    <script>
        function passvisToggle() {
            let x = document.getElementById("pw-input");

            if (x.type === "password") {
                document.getElementById("pw-eye").style.display = "none";
                document.getElementById("pw-eye-slash").style.display = "block";
                x.type = "text";
            } else {
                document.getElementById("pw-eye").style.display = "block";
                document.getElementById("pw-eye-slash").style.display = "none";
                x.type = "password";
            }
        }

        // function removeEye() {
        //     let container = document.querySelector(".card");
        //     for (i = 0; i < container.length; i++) {
        //         if (container[i].value === "The email field is required." && container[i].value === "The password field is required.") {
        //             return document.getElementById("pw-eye").style.display = "none";
        //             return document.getElementById("pw-eye-slash").style.display = "none";
        //         }
        //     }
        // }
    </script>
@endsection
